...
function printer.make_routes( server )
	...
	server:get('/api/state',function( request , response )
		response:json(printer_api:get_state())
	end)

	 server:get('/api/terminal',function( request , response)
	    response:json(printer_api:get_terminal_log(tonumber(request.query.from)))
	end)
	server:post('/api/terminal/send',function( request , response)
	    response:json(printer_api:send_cmd(request.json.cmd))
	end)

	server:post('/api/disconnect',function( request , response)
	    response:json(printer_api:disconnect())
	end)
	server:post('/api/connect',function( request , response)
	    response:json(printer_api:connect())
	end)
	server:post('/api/pause',function( request , response )
	    response:json(printer_api:pause())
	end)
	server:post('/api/resume',function( request , response )
	    response:json(printer_api:resume())
	end)
	...
	server:post('/api/flash_firmware',function( request , response )
	    response:json(printer_api:flash_firmware(request.query.file))
	end)
	server:post('/api/action',function( request , response)
	    response:json(printer_api:action(request.json.action,request.json.data))
	end)
end
