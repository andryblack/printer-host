...
function Parser:on_block( begin_ext, block, end_ext )
	--print('block:',block)
	if begin_ext then
		assert(not self._extended_command,'extended command already started')
	...
	if end_ext then
		assert(self._extended_command,'extended command not started, but close')
...
function Parser._EC.AM( self, data )
	print('AM',data)
end
