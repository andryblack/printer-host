...
bool Serial::configure_baud(lua_State* L,int baud,bool parity) {
	if (m_fd<0)
		return false;
	int ttyfd = m_fd;
	...
	options.c_cflag &= ~CSTOPB; // 8x1
	options.c_cflag &= ~CSIZE;
	options.c_cflag |= CS8;
	options.c_cflag &= ~CRTSCTS;

	 options.c_iflag &= ~IGNBRK;
	 options.c_iflag &= ~(INLCR | IGNCR | ICRNL | IUCLC | INPCK | IGNPAR | PARMRK); // no char processing
