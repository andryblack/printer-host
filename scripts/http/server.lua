...
function server:_init( settings )
	...
	self._web:set_fs_root(self._config.rootdir)
	local static = self._web:use(web.static('public',{extensions={'js','css','html','map','svg'}}))
	self._web:use(web.json())
	self._web:use(web.cookie())
	self._web:use(web.formparser())
	self._web:use(web.multipart())
	self._web:use(web.views('view',{ext='html',nocache=true}))

	self._web:get('/',function( request , reply)
		return reply:redirect('/home')
	end)

	for _,v in ipairs(self.sidebar) do
		self._web:get('/' .. v.name,function(req,res)
...
function server:post(...)
	return self._web:post(...)
end
...
-- local generator_api = require 'http.api.generator'
-- generator_api.make_routes( server )

-- local settings_api = require 'http.api.settings'
-- settings_api.make_routes( server )
